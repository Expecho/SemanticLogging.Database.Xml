...
{
    /// <summary>
    /// Factories and helpers for using the <see cref="XmlSqlDatabaseSink"/>.
    /// </summary>
    public static class XmlSqlDatabaseLog
    {
        /// <summary>
        ...
        /// <param name="onCompletedTimeout">Defines a timeout interval for when flushing the entries after an <see cref="XmlSqlDatabaseSink.OnCompleted"/> call is received and before disposing the sink.
        ...
        public static SinkSubscription<XmlSqlDatabaseSink> LogToSqlDatabase(this IObservable<EventEntry> eventStream, string instanceName, string connectionString, string tableName = DefaultTableName, string storedProcedureName = DefaultStoredProcedureName, TimeSpan? bufferingInterval = null, int bufferingCount = Buffering.DefaultBufferingCount, TimeSpan? onCompletedTimeout = null, int maxBufferSize = Buffering.DefaultMaxBufferSize)
        {
            ...
            return new SinkSubscription<XmlSqlDatabaseSink>(subscription, sink);
        }

        /// <summary>
        /// Subscribes to the listener using a <see cref="XmlSqlDatabaseSink" />.
        /// </summary>
        /// <param name="instanceName">The name of the instance originating the entries.</param>
        /// <param name="connectionString">The connection string.</param>
        /// <param name="tableName">The name of the table.</param>
        ...
        /// the <see cref="System.Diagnostics.Tracing.EventListener" /> will block until all the entries are flushed or the interval elapses.
        /// If <see langword="null" /> is specified, then the call will block indefinitely until the flush operation finishes.</param>
        /// <returns>An event listener that uses <see cref="XmlSqlDatabaseSink"/> to log events.</returns>
